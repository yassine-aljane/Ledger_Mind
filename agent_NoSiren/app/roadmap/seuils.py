...
Le YAML est lu une seule fois (cache mémoire) ; `reload()` force une relecture (tests).
...
# data/seuils.yaml à la racine du projet (…/app/roadmap/seuils.py -> racine = parents[2])
_YAML_PATH = Path(__file__).resolve().parents[2] / "data" / "seuils.yaml"
...
def reload() -> dict:
    """Vide le cache et relit le fichier (utile si le YAML change à chaud / en test)."""
    _load.cache_clear()
    return _load()
