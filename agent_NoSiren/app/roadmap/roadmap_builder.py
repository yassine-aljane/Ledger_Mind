...
from app.roadmap import comparateur as C
from app.roadmap import presentation as P
from app.roadmap import seuils as S
from app.roadmap.analyse_juridique import DURABILITE_INDET
from app.roadmap.models import Fraicheur, Meta, Roadmap
from app.roadmap.presentation import PARCOURS_BASCULE, PARCOURS_MICRO, PARCOURS_SOCIETE
...
# ---------------------------------------------------------------------------
#  Décision (compat) : analyse juridique + parcours UX en une structure
# ---------------------------------------------------------------------------
def decide_regime(profil: dict) -> dict:
    """Décision déterministe : combine l'analyse juridique et le parcours UX.

    Conservé pour compat ; expose aussi l'objet `analyse` (AnalyseJuridique) pour les appelants
    qui veulent le verdict légal pur découplé de l'affichage.
    """
...
# ---------------------------------------------------------------------------
#  Verdict injectable comme contrainte dans les réponses LLM
# ---------------------------------------------------------------------------
...
def verdict_regime(profil: dict) -> dict | None:
    ...
    None si le CA est inconnu/nul (aucune décision possible, donc aucune contrainte imposée).
    """
