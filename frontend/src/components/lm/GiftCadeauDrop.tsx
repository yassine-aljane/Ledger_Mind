 ...
 * `variant="panel"` : dans la carte Déposer (onglet).
 *
 * Dès qu'une photo est choisie, elle REMPLACE la scène : l'utilisateur voit la pièce
 * qu'il déclare, pas un carton décoratif.
 */
...
type Props = {
  ...
  /** Désactive le clic pendant une analyse en cours. */
  disabled?: boolean;
  /** Object URL de la photo choisie — affichée à la place de la scène. */
  preview?: string | null;
  /** Nom du fichier choisi, rappelé sous l'aperçu. */
  fileName?: string | null;
  /** Analyse en cours : balayage sur l'aperçu. */
  busy?: boolean;
};
...
function GiftCube({ reveal, shadow = false }: { reveal: boolean; shadow?: boolean }) {
  return (
    <div className={cn("lm-gift-cube", reveal && "lm-gift-cube--open")} aria-hidden>
      <div className="lm-gift-cube__scene">
        <div className="lm-gift-cube__lid">
      ...
      {shadow && <div className="lm-gift-cube__shadow" />}
    </div>
  );
}

/** Aperçu de la pièce importée, à la place de la scène. */
function GiftPhoto({
  src,
  busy,
  rail = false,
  ...
  return (
    <span
      className={cn(
        "lm-gift-photo",
        rail && "lm-gift-photo--rail",
        busy && "lm-gift-photo--busy",
      )}
    >
      <img
        src={src}
        alt="Photo importée du cadeau"
        className="lm-gift-photo__img"
        draggable={false}
      />
    </span>
  );
    ...
    <div className={cn("relative", className)}>
      ...
      <button
        ...
        className={cn(
          ...
          panel && "lm-gift-portal--panel",
          reveal && "lm-gift-portal--open",
          showPhoto && "lm-gift-portal--photo",
          disabled && "pointer-events-none opacity-70",
        )}
        style={sceneTilt}
        onMouseEnter={() => setHover(true)}
        ...
        aria-label={
          showPhoto
            ? "Remplacer la photo du cadeau ou de la dotation"
            : "Importer une photo du cadeau ou de la dotation"
        }
      >
        {reveal &&
          !rail &&
        ...
        {rail ? (
          <div className="relative z-10 flex items-center gap-4 px-4 py-3.5 sm:gap-5 sm:px-5">
            {showPhoto ? (
              <GiftPhoto src={preview as string} busy={busy} rail />
            ) : (
              <div
                className={cn("lm-gift-stage lm-gift-stage--rail", reveal && "lm-gift-stage--open")}
              >
                <GiftCat reveal={reveal} />
                <GiftCube reveal={reveal} />
              </div>
            )}

            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-ink-foreground">Cadeau / dotation</p>
              <p className="mt-0.5 truncate text-xs leading-snug text-ink-foreground/55">
                {showPhoto
                  ? busy
                    ? "Lecture de la photo…"
                    : (fileName ?? "Cliquez pour changer de photo")
                  : reveal
          ...
          <div
            className={cn(
              "relative z-10 flex flex-col items-center px-5 pb-8 pt-8",
              showPhoto ? "justify-center" : "justify-end",
              panel ? "min-h-64 sm:min-h-72" : "min-h-72 sm:min-h-80",
            )}
          >
            {showPhoto ? (
              <GiftPhoto src={preview as string} busy={busy} />
            ) : (
              <>
                <GiftCat reveal={reveal} />
                <div className={cn("lm-gift-stage", reveal && "lm-gift-stage--open")}>
                  <GiftCube reveal={reveal} shadow />
                </div>
              </>
            )}
            <div
              id={`${uid}-hint`}
              className={cn("relative z-20 max-w-xs text-center", showPhoto ? "mt-5" : "mt-6")}
            >
              <p className="lm-gift-copy font-display text-xl text-ink-foreground">
                {showPhoto
                  ? busy
                  ...
                  : reveal
                    ? "Importez une photo"
                    : "Un cadeau à déclarer ?"}
              </p>
              <p className="mt-1.5 truncate text-xs text-ink-foreground/55">
                {showPhoto
                  ? busy
                    ? "Reconnaissance de l'objet en cours"
                    : (fileName ?? "Cliquez pour choisir une autre image")
                  : reveal
